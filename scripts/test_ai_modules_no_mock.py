...
"""
AI模块专项测试脚本（无MOCK版本）
专门测试LangChain AI模块的功能，使用真实的AI提供商
"""
import sys
import time
import logging
...
class AIModuleTesterNoMock:
    ...
    def run_ai_tests(self, test_level="all", save_logs=True):
        """运行AI模块测试"""
        logger.info("🤖 TestMind AI - AI模块专项测试（无MOCK版本）")
        logger.info("=" * 60)
    ...
    def _test_all_providers_initialization(self):
        ...
        try:
            ...
            # 测试Gemini提供商
            gemini_extractor = LangChainExtractor(
    ...
    def _test_provider_configuration(self):
        ...
        try:
            ...
            self._record_success(test_name, "提供商配置验证成功")
    ...
    def _save_test_logs(self, test_level, duration):
        ...
        try:
            ...
            log_file = self.reports_dir / f"ai_module_no_mock_{test_level}_{timestamp}.log"
            
            # 收集所有日志信息
            log_content = []
            log_content.append(f"🤖 TestMind AI - AI模块专项测试日志（无MOCK版本）")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="AI模块专项测试（无MOCK版本）")
    parser.add_argument(
        "--level",
